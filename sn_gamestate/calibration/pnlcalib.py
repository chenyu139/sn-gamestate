...
from pnlcalib.utils.utils_heatmap import (get_keypoints_from_heatmap_batch_maxpool, \
...
class PnLCalib_Keypoints(ImageLevelModule):
    ...
    output_columns = {
        "image": ["keypoints", "lines_det", "lines"],
        "detection": []
    }

    def __init__(self, checkpoint_kp, checkpoint_l, image_width, image_height, batch_size, device, \
                    cfg, cfg_l, **kwargs):
    ...
    def process(self, batch: Any, detections: pd.DataFrame, metadatas: pd.DataFrame):
        ...
        kp_dict = coords_to_dict(kp_coords, threshold=0.1611, ground_plane_only=True)
        lines_dict = coords_to_dict(line_coords, threshold=0.3434, ground_plane_only=True)

        image_width = batch.size()[-1]
        image_height = batch.size()[-2]
        kp_dict, lines_dict = complete_keypoints(kp_dict[0], lines_dict[0], w=image_width, h=image_height, normalize=True)

        output_pred = []

        output_pred.append(pd.Series({"keypoints": kp_dict, "lines_det": lines_dict,
